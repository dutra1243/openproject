        ...
        module Validators
          class AuthenticationValidator < HealthReports::ValidatorGroup
            ...
            def user_bound_request
              # TODO: how to perform this call for real? how to authenticate?
              access_token = OAuthClientToken.for_user_and_client(user, subject.oauth_client).first&.access_token

              subject.resolve("queries.user")
                .call(Wikis::Adapters::Input::UserQuery.new(access_token:))
                .or { fail_check(:user_bound_request, oauth_request_error_code(it)) }
